---
name: "customer-business-review"
version: "3.0.2"
description: "生成客户飞书整体使用情况回顾与数据洞察画板。用户提到回访、BR、Business Review、客户复盘、使用回顾或主租户近 180 天分析等相近意图时自动调用。"
---

# 飞书整体使用情况回顾

以客户主租户数据为起点，生成飞书整体使用情况回顾和一张客户可见数据洞察画板。支持 C360 最新快照和 C360 + Aeolus 近 180 天增强两种模式。

## 调用边界

- 当用户提到回访、BR、Business Review、客户复盘、使用回顾、飞书使用分析或数据洞察画板等相近意图时自动执行，无需用户明确点名本 Skill。
- 创建、评审或测试本 Skill 时不得查询真实客户；只做静态校验或使用明确标注的虚构数据。
- 运行时只读查询，不写回 C360，不自动向客户发消息，不自动分享文档。
- 当前调用者视为 CSM；文档与画板创建在其个人云盘个人空间。
- **交付到“飞书整体使用情况回顾”即结束。** 不生成案例、产品速递、服务计划、场景共创、内外双版本、交互卡片、一键定稿或会后自动更新。

## 输入

1. `customer_name` 或 `tenant_fcode` 至少提供一项；两者都有时交叉校验。
2. 可选 `review_reason`：仅用于理解回顾背景，不进入数据画板。
3. 可选 `review_month`：计划回访月份；缺失时使用当前月份。
4. 用户消息、任务评论或 resource 中已提供的客户身份、主租户和七模块结构化字段均视为有效输入。

输入优先级：

1. 当前消息正文中明确列出的结构化字段；
2. 当前任务可读取的 resource / Drive 文件；
3. Aily workspace 持久化副本；
4. 重新调用 C360 查询。

正文数据已覆盖七模块时立即进入 coverage matrix 和产出，不得要求用户再次粘贴，不得因 resource 不可读而忽略正文。

先执行 [环境检测、安装与恢复](references/bootstrap-and-recovery.md) 和 [运行能力检测与用户提醒](references/runtime-capability-and-notification.md)。若未安装 `lark-c360`，必须使用 Skill 内置脚本从 C360 官方 TOS 安装；公共 npm 404 不是终止条件。启动时识别运行环境，并选择 C360 快照模式或 C360 + Aeolus 增强模式。

若输入 F 码，先精确反查租户和所属客户，再查询该客户下全部租户并按 DAU 验证主租户。租户显示名与客户正式名称分开保存。客户无法唯一匹配时，要求 CSM 补充或选择，并停止其他动作。

## 强制执行顺序

严格遵循 [执行流程](references/workflow.md)、[数据源与工具路由](references/tool-routing.md)、[运行能力检测与用户提醒](references/runtime-capability-and-notification.md)、[Aeolus 浏览器自动化手册](references/aeolus-browser-runbook.md) 和 [参考 BR 与数据完整性门禁](references/reference-br-and-completeness.md)：

1. 在 C360 唯一匹配客户，取得关联租户、DAU、主租户与 F 码。
2. 以 DAU 最高的关联租户作为唯一主租户；其他租户不得参与汇总、对比或归因。
3. 先补齐 C360 七模块固定字段，并确认是否足以生成当前使用快照。
4. 若运行环境可访问 Aeolus，或用户提供 Aeolus 导出，则用主租户 **F 码**获取滚动 180 天累计、日均和对比期指标；不得用客户名或租户名替代 F 码。
5. 建立 coverage matrix，按七模块组合分析全部有效指标。
6. C360 快照模式必须按 [确定性交付流水线](references/deterministic-delivery.md) 生成；禁止 Agent 手写 SVG、单位或洞见。没有 `delivery-receipt.json` 时禁止创建文档或返回交付链接。
7. 回读画板和文档，确认数据、结构、口径和视觉无误后结束。

若任务方声明 C360 数据位于旧 `/home/gem/.aily/workdir/...` 路径，但当前沙箱不可读：先解析 `~/.aily/workspace` 持久化副本；不存在时安装/登录 C360 并重新查询。不得因旧任务路径失效而停止。

阻塞条件仅包括：客户无法唯一匹配；主租户或 F 码缺失；C360 七模块数据不足以形成有效回顾。Aeolus 不可访问本身不阻塞 C360 快照版。

当七模块各有至少 3 个有效字段，且客户、主租户和 F 码已明确时，输入门禁已通过。此后禁止回复“请把完整字段贴过来”，必须继续创建画板和文档。

正式快照版不是“字段够 3 个即可自由生成”。进入产出后必须补齐 35 个必需字段，并把 C360 实体 meta 中真实存在、已注册且有有效值的扩展字段写入 `extra_metrics`。35 个字段是下限而不是上限；禁止删掉会议数、参会人次、VC DAU 等真实字段，也禁止模型猜值。

所有对外展示值统一四舍五入为整数，原始 JSON 小数必须保留在输入和来源账本中。百分比输入统一使用 `0_to_100` 口径，不得无条件乘以 100。

## 数据与分析

按照 [数据与洞见规则](references/data-and-insights.md) 执行，以下规则不可降级：

- **C360 快照模式**：当 Aily、云端沙箱或其他运行环境无法通过浏览器访问 ByteDance 内网 Aeolus 时，使用 C360 七模块最新快照生成正式回顾。标题和口径必须明确写 `C360 最新使用快照`，不得声称覆盖近 180 天，不展示累计、日均、趋势或同比。
- **C360 + Aeolus 增强模式**：TRAE、Codex 或其他 Agent 具备可访问内网的模拟浏览器、内置浏览器或用户 Chrome 自动化时，自动操作 Aeolus；用户提供 CSV/XLSX、完整截图或已确认指标表时也可进入增强模式。
- 不得因为 Aeolus 不可访问而反复重试、要求 Aily 接管本地浏览器，或在 C360 七模块已完整时停止整个任务。
- Aily 按 [Aeolus 数据交接](references/aeolus-handoff.md) 只提醒一次并立即交付 C360 快照版；用户后续补充数据时再升级现有文档和画板。
- 增强模式的当前周期以 Aeolus 最近可用数据日为结束日滚动 180 天；同比周期是紧邻的前一个 180 天。
- 后台分析全部可用指标，画板按模块只选 3～5 个关键指标。回顾正文必须展示所有有效核心指标和完整模块数据，不受该数量限制。
- 生成回顾文档前必须建立 coverage matrix，并按 [参考 BR 与数据完整性门禁](references/reference-br-and-completeness.md) 校验；任一有效核心字段未展示时不得宣称完成。
- 画板默认遵循七个独立一级模块：即时协同、会议协同、内容沉淀、多维表格、知识管理、AI 赋能、服务台。知识管理不得并入 AI 赋能，内容沉淀不得替代知识管理。
- 七个序号模块必须使用单列通栏布局，每个模块独占一行，不使用两列并排卡片。
- 模块标题统一采用 `01｜统一协同底座、工作入口稳定` 格式，序号后必须使用全角分隔符 `｜`。
- 禁止使用白色圆角方块包裹单字或缩写；七个模块必须各有一个无底框、简洁且可见的线性 SVG 矢量图标。
- 每个模块的数据区必须使用 5 等分固定栅格；所有行复用相同的 5 个列中心点。
- 指标名称和指标值必须在各自列内水平居中，并共享同一中心轴。
- 指标名称不带括号单位；单位必须跟随数字写在同一文本节点，例如 `648 人`、`46.73 分钟`、`542,600 次`。
- 只有增强模式可以写 `近 180 天`，并禁止改写成“半年”“近半年”或“6 个月”；快照模式只写字段自身口径。
- 画板不展示洞见；每个模块的洞见只写入文档正文，并必须包含一个可复核的数据关系。
- 禁止使用“高效流转”“价值显著”“持续提升”等没有数据关系支持的口号替代洞见。
- 洞见计算可在后台使用原始值和派生值，文档洞见不得逐项复述表格。
- 文档洞见只写由数据关系推导出的结构判断、阶段判断、覆盖差距或使用特征。
- 必须形成“指标组合 → 客观事实 → 业务含义”的组合洞见，不得逐项复述指标。
- 只写客观事实和谨慎的业务含义，不输出原因假设。
- 快照模式的 AI 模块分析当前规模、产品分布和集中度；增强模式可进一步分析趋势、渗透、人均和场景。无有效数据则隐藏模块。

运行时按 [数据源与工具路由](references/tool-routing.md) 调用现有 Skill。不得用通用搜索或自造请求替代 C360、Aeolus、飞书文档和画板的规定入口。

## 交付物

按照 [交付物与生命周期](references/deliverables-and-lifecycle.md) 创建：

- **单一数据指标画板**：七模块摘要、统计数据和口径说明，不展示洞见。
- **飞书整体使用情况回顾文档**：合规说明、核心结论、同一张画板、核心指标总览、七模块完整数据与客观判断。

文档命名默认：`主租户显示名称｜飞书整体使用情况回顾｜YYYY-MM`。用户明确指定名称时以用户要求为准。

## 异常与降级

- C360 局部缺失：仅对有效字段生成洞见，并在口径说明中标记缺失范围。
- Aeolus 不可访问：自动使用 C360 快照模式，正常生成文档与画板；只在结果中简短说明未包含近 180 天累计、日均和趋势。
- 模块无有效数据：隐藏模块，不展示空状态。
- 同比基期无数据：只展示当前值，不展示同比。
- 任何无数据或无权限的情况：明确记录缺口，禁止编造。

## 完成检查

交付前逐项确认：

- 已唯一匹配客户，且只分析 DAU 最高主租户。
- 已明确标记运行模式和数据源。
- 若为 C360 快照模式，未出现“近 180 天累计、日均、趋势、同比”等无来源表述。
- 若为增强模式，已使用主租户 F 码回填 Aeolus，且当前期与对比期日期正确。
- 文档洞见来自指标组合，事实与原因假设分离；画板不展示洞见。
- 只有一张总画板，且画板不含归因、内部资料或风险。
- 文档仅包含“飞书整体使用情况回顾”，没有案例、服务计划或其他后续章节。
- 文档与画板引用同一组数据和口径。
- 未写回 C360，未编造数据、对标或因果关系。
- 已执行 `render-snapshot.py` 和 `audit-snapshot.py`；云端 raw 画板 `image=0` 且图标分组不少于 7。
