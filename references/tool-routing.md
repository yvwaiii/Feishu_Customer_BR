...
- Aeolus 通过浏览器访问指定看板，不使用 WebFetch、curl 或自行拼接接口替代页面查询。
- 只有获取主租户 F 码并成功回填 Aeolus 后，才能启动完整分析。
...
必须调用浏览器能力，并按 [Aeolus 浏览器自动化手册](aeolus-browser-runbook.md) 完成：
...
`唯一匹配客户 → 查询关联租户 → 按 DAU 选择主租户 → 取得 F 码 → 回填 Aeolus 并确认查询成功`
...
汇总分析必须等待 C360 与 Aeolus 两个核心数据源都返回有效结果。
